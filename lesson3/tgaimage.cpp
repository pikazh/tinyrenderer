...
TGAImage::TGAImage(const int w, const int h, Format bpp) : w(w), h(h)
{
    this->bpp = static_cast<int>(bpp);
    this->data.resize(w * h * this->bpp, 0);
}
...
bool TGAImage::read_tga_file(const std::string filename)
{
    ...
    if (w <= 0 || h <= 0 || (bpp != static_cast<int>(Format::GRAYSCALE) && bpp != static_cast<int>(Format::RGB) && bpp != static_cast<int>(Format::RGBA)))
    {
        in.close();
...
bool TGAImage::write_tga_file(const std::string filename, const bool vflip, const bool rle) const
{
    ...
    header.width = w;
    header.height = h;
    header.datatypecode = (bpp == static_cast<int>(Format::GRAYSCALE) ? (rle ? 11 : 3) : (rle ? 10 : 2));
...
TGAColor TGAImage::color_at(const int x, const int y) const
{
...
void TGAImage::set_color_at(int x, int y, const TGAColor &c)
{
    if (!data.size() || x < 0 || y < 0 || x >= w || y >= h)
        return;
