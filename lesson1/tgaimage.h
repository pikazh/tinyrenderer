...
struct TGAImage
{
    ...
    TGAImage() = default;
    TGAImage(const int w, const int h, Format bpp);
    bool read_tga_file(const std::string filename);
    bool write_tga_file(const std::string filename, const bool vflip = true, const bool rle = true) const;
    void flip_horizontally();
    void flip_vertically();
    TGAColor color_at(const int x, const int y) const;
    void set_color_at(const int x, const int y, const TGAColor &c);
    int width() const;
    int height() const;
